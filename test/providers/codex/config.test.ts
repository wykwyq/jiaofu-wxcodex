...
test('loadCodexProfilesFromEnv can make Codex honor the system proxy', () => {
  const result = loadCodexProfilesFromEnv({
    CODEX_REAL_BIN: '/usr/bin/codex',
    CODEX_APP_RESPECT_SYSTEM_PROXY: '1',
  });

  assert.deepEqual(result.profiles[0]?.config.codexCliArgs, [
    '--enable',
