...
export class CodexAppClient extends EventEmitter {
  ...
  constructor({
    ...
    turnTimeoutMs = resolveTurnTimeoutMs(process.env.CODEXBRIDGE_TURN_TIMEOUT_MS),
  ...
  private async interruptTimedOutTurn({ threadId, turnId }: { threadId: string; turnId: string }): Promise<void> {
    try {
      await this.request('turn/interrupt', { threadId, turnId }, { timeoutMs: 5_000 });
      this.logDebug('turn_timeout_interrupted', { threadId, turnId });
    } catch (error) {
      this.logDebug('turn_timeout_interrupt_failed', {
        threadId,
        turnId,
        error: error instanceof Error ? error.message : String(error),
      });
...
function resolveTurnTimeoutMs(value: unknown): number {
  const parsed = Number.parseInt(String(value ?? '').trim(), 10);
  return Number.isInteger(parsed) && parsed >= 10_000
    ? parsed
