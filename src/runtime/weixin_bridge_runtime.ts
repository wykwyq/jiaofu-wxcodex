...
export class WeixinBridgeRuntime {
  ...
  async processInboundEventWithOptions(
    ...
    let processingNoticeTimer: ReturnType<typeof setTimeout> | null = null;
    let responseSettled = false;
    if (!parseSlashCommand(String(event?.text ?? '')) && this.processingNoticeDelayMs > 0) {
      processingNoticeTimer = setTimeout(() => {
        if (responseSettled) {
          return;
        }
        void this.sendTextWithRetry({
          externalScopeId: event.externalScopeId,
          content: this.i18n.t('runtime.turn.processing'),
        }).catch(async (error) => {
          await this.onError(error);
        });
      }, this.processingNoticeDelayMs);
    }
    try {
